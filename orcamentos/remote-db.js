'use strict';
(() => {
  const HISTORY_API='https://73ee0db2-d333-4cc6-9729-ca6149cffff7.created.app';
  ...
  Object.assign(window.MULTPREST_DATA_SOURCES,{
    historico:{api:HISTORY_API,mode:'local-first-until-auth-gateway'},
  ...
  function panel(){
    ...
    p.innerHTML=`<div class="hist-remote-main"><span class="hist-remote-dot" id="histRemoteDot"></span><div><b>Banco privado — Histórico original</b><small id="histRemoteStatus">Verificando conexão com o sistema original...</small></div></div><div class="hist-remote-actions"><button class="btn ghost tiny" id="histRemoteCheck">Testar conexão</button><button class="btn ghost tiny" id="histRemoteView" disabled>Ver registros remotos</button></div>`;
  ...
  async function checkHistory(){
    setStatus(null,'Verificando conexão...');
    try{
      const r=await fetch(`${HISTORY_API}/api/orcamentos`,{headers:{Accept:'application/json'},cache:'no-store',credentials:'omit'});
      if(!r.ok)throw new Error(`HTTP ${r.status}`);
      const data=await r.json();remoteRows=Array.isArray(data)?data:[];
      setStatus(true,`Conectado ao histórico original • ${remoteRows.length} orçamento(s) no banco`);
    }catch(e){
      console.warn('Histórico remoto ainda requer gateway/autenticação',e);remoteRows=[];
      setStatus(false,'Banco identificado, mas o navegador não recebeu permissão de acesso direto. O histórico local continua ativo até habilitarmos o login/gateway seguro.');
    }
  }
  ...
  function showRemote(){
    ...
    modal.innerHTML=`<div class="hist-remote-box"><div class="hist-remote-head"><div><b>Registros do banco original</b><small>Consulta somente. Nenhum orçamento local é alterado.</small></div><button id="histRemoteClose">×</button></div><div class="hist-remote-list">${remoteRows.slice(0,100).map(x=>`<div><span><b>${esc(x.numero||'—')}</b><small>${esc(x.descricao||'Sem descrição')}</small></span><span><b>${x.is_fechado?'Fechado':'Aberto'}</b><small>${fmt(x.created_at)}</small></span></div>`).join('')}</div></div>`;
  ...
  const style=document.createElement('style');style.textContent=`.hist-remote-panel{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:11px 13px;margin:0 0 12px}.hist-remote-main,.hist-remote-actions{display:flex;align-items:center;gap:9px}.hist-remote-main b{display:block;font-size:10px}.hist-remote-main small{display:block;color:#94a3b8;font-size:9px;margin-top:2px;max-width:780px}.hist-remote-dot{width:9px;height:9px;border-radius:50%;background:#f59e0b;box-shadow:0 0 0 4px rgba(245,158,11,.13);flex-shrink:0}.hist-remote-dot.ok{background:#10b981;box-shadow:0 0 0 4px rgba(16,185,129,.13)}.hist-remote-dot.bad{background:#ef4444;box-shadow:0 0 0 4px rgba(239,68,68,.13)}.hist-remote-modal{position:fixed;inset:0;z-index:100;background:rgba(2,6,23,.75);display:grid;place-items:center;padding:18px}.hist-remote-box{width:min(760px,100%);max-height:80vh;overflow:auto;background:#0b1728;border:1px solid #334155;border-radius:14px;padding:15px}.hist-remote-head{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px}.hist-remote-head b{display:block}.hist-remote-head small{display:block;color:#94a3b8;font-size:9px;margin-top:3px}.hist-remote-head button{border:0;background:transparent;color:#cbd5e1;font-size:22px;cursor:pointer}.hist-remote-list{display:grid;gap:7px}.hist-remote-list>div{display:flex;justify-content:space-between;gap:14px;padding:9px 10px;border:1px solid #25364c;border-radius:9px;background:#0f1c30}.hist-remote-list span:last-child{text-align:right}.hist-remote-list b{display:block;font-size:10px}.hist-remote-list small{display:block;color:#64748b;font-size:8px;margin-top:2px}@media(max-width:700px){.hist-remote-panel{align-items:flex-start;flex-direction:column}.hist-remote-actions{width:100%}}`;
