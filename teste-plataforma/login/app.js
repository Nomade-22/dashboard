...
(()=>{
 ...
 async function check(key){const r=await fetch(`${API}/auth-check`,{method:'POST',headers:{Authorization:`Bearer ${key}`,Accept:'application/json'},cache:'no-store'});let data={};try{data=await r.json()}catch{}if(!r.ok)throw new Error(data.error||'Não foi possível validar o acesso');return data}
