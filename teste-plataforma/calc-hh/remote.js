...
(() => {
  ...
  const KEY_COMMERCIAL='multprest_calc_hh_commercial_rates_v1';
  const KEY_ORC_RATES='multprest_orc_labor_rates_v1';
  const KEY_ORC_SOURCES='multprest_orc_labor_rates_sources_v1';
  let remote=null;
  ...
  function normalizeCommercial(data){
    const rates={};
    if(!Array.isArray(data?.valores))return rates;
    for(const v of data.valores){
      const cargoId=String(v?.cargo?.id||v?.cargoId||'');
      const empresaId=String(v?.empresa?.id||v?.empresaId||'');
      const worker=CARGO_TO_WORKER[cargoId],client=EMPRESA_TO_CLIENT[empresaId],value=Number(v?.valorHora)||0;
      if(worker&&client&&value>0)rates[`${worker}|${client}`]=value;
    }
    return rates;
  }

  function publishCommercial(data){
    const rates=normalizeCommercial(data);
    localStorage.setItem(KEY_COMMERCIAL,JSON.stringify({source:'calc-hh',syncedAt:new Date().toISOString(),rates}));
    window.dispatchEvent(new CustomEvent('multprest:commercial-rates-ready',{detail:{rates}}));
    return rates;
  }

  function syncCommercialToBudget(){
    const pack=read(KEY_COMMERCIAL,{rates:{}}),srcRates=pack.rates||{};
    const rates=read(KEY_ORC_RATES,{}),sources=read(KEY_ORC_SOURCES,{});
    let updated=0,preserved=0;
    for(const [key,valueRaw] of Object.entries(srcRates)){
      const value=Number(valueRaw)||0;if(value<=0)continue;
      const local=Number(rates[key])||0,meta=sources[key],tracked=meta?.source==='calc-hh';
      if(local>0&&!tracked&&!nearly(local,value)){preserved++;continue;}
      if(!nearly(local,value)){rates[key]=value;updated++;}
      sources[key]={source:'calc-hh',value,updatedAt:new Date().toISOString()};
    }
    localStorage.setItem(KEY_ORC_RATES,JSON.stringify(rates));
    localStorage.setItem(KEY_ORC_SOURCES,JSON.stringify(sources));
    localStorage.setItem('multprest_orc_rates_last_sync_v1',new Date().toISOString());
    return {updated,preserved,total:Object.keys(srcRates).length};
  }
  ...
  async function loadRemote(){
    ...
    remote=data;localStorage.setItem(KEY_CACHE,JSON.stringify({syncedAt:new Date().toISOString(),data}));publishCommercial(data);return data;
  }
  ...
  async function refresh(){
    ...
    catch(e){console.warn(e);remote=cached();if(remote)publishCommercial(remote);status(false,remote?'Banco indisponível agora — exibindo última cópia':'Sem acesso ao banco agora — cálculos locais preservados');renderRemoteTable()}
  ...
  function renderRemoteTable(){
    ...
    table.innerHTML=h;
    const wrap=table.closest('.table-wrap');if(wrap&&!wrap.previousElementSibling?.classList?.contains('remote-table-note')){const n=document.createElement('span');n.className='remote-table-note';n.textContent='Fonte: banco original do Calc HH • TARIFA COMERCIAL de venda por Cargo + Cliente';wrap.before(n)}
  }
  ...
  const old=cached();if(old)publishCommercial(old);
  inject();refresh();
})();