...
(() => {
  ...
  function laborSyncValues(){
    const items=(load(KEY_LABOR,[])||[]).filter(i=>num(i.people)>0&&num(i.days)>0);
    const laborTeamSize=Math.min(9,Math.max(1,num(localStorage.getItem(KEY_TEAM))||9));
    const totalPeople=items.reduce((s,i)=>s+num(i.people),0);
    const personDays=items.reduce((s,i)=>s+num(i.people)*num(i.days),0);
    const uniqueDays=[...new Set(items.map(i=>num(i.days)))];
    const uniformDays=uniqueDays.length===1?uniqueDays[0]:null;
    const canSync=items.length>0&&uniformDays!==null;
    return {laborTeamSize,totalPeople,personDays,uniformDays,canSync};
  }
  function syncFirst(kind){
    const sync=laborSyncValues();
    if(!sync.canSync){
      alert(sync.personDays>0?'A Mão de Obra possui funções com quantidades de dias diferentes. Para não calcular alimentação/hospedagem errado, crie itens separados em Despesas ou preencha pessoas e dias manualmente.':'Preencha pessoas e dias na Mão de Obra antes de puxar para Despesas.');
      return;
    }
    state.teamSize=sync.laborTeamSize;const arr=state[kind];if(!arr?.length)return;
    arr[0]={...arr[0],people:sync.totalPeople,days:sync.uniformDays};saveAndRender();
  }
