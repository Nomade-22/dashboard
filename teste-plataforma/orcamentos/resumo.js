...
(() => {
  ...
  function render(){
    ...
    // Somente categorias atômicas entram nos percentuais e no gráfico.
    // despesasViagem é um subtotal informativo e não pode ser somado novamente.
    const cards=[['👷','Mão de Obra',t.maoDeObra,'orange'],['📦','Material',t.material,'blue'],['🏠','Estadia',t.estadia,'purple'],['🚗','Transporte',t.transporte,'cyan'],['☕','Alimentação',t.alimentacao,'amber'],['🕒','Horas Viajadas',t.viagens,'rose']];
    ...
    section.innerHTML=`
      ...
      <section class="card summary-detail"><div class="card-title"><h3>🧾 Detalhamento Completo</h3></div>
        ...
        <div class="sum-row"><span>💼 Subtotal despesas de viagem</span><strong>${money(t.despesasViagem)}</strong></div>
        <div class="sum-total"><span>🧮 TOTAL GERAL</span><strong>${money(t.grand)}</strong></div>
      </section>
      <section class="card summary-chart"><div class="card-title"><h3>↗ Distribuição do Orçamento</h3></div>${cards.map(([,label,value,color])=>`<div class="chart-row"><div><span>${label}</span><b>${pct(value,t.grand).toFixed(1)}%</b></div><div class="chart-track"><i class="${color}" style="width:${Math.min(pct(value,t.grand),100)}%"></i></div></div>`).join('')}</section>`;
  }
