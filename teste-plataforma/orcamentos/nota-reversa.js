...
(() => {
  ...
  function calc(){
    ...
    const issqn=state.incluirIssqn?gross*(num(state.issqnPercent)/100):0;
    const inss=state.incluirInss?gross*(num(state.inssPercent)/100):0;
    const simples=state.incluirSimples?gross*(num(state.simplesPercent)/100):0;
  ...
  const taxRow=(key,label,pct,value)=>`<div class="nr-toggle-row"><label><input type="checkbox" data-state="${key}" ${state[key]?'checked':''}><span class="nr-switch"></span><span>${label} <small>${num(pct).toFixed(2)}%</small></span></label><strong class="${state[key]?'deduct':''}">${state[key]?'- ':''}${money(value)}</strong></div>`;
  ...
  function render(){
    ...
    section.innerHTML=`
      <div class="page-title">
        <div><span class="eyebrow">Análise líquida</span><h2>Nota Reversa</h2><p>Impostos sobre o valor bruto, antecipação sobre o líquido após impostos e dedução dos custos reais sem BDI. Os percentuais abaixo pertencem a este orçamento e devem refletir a condição real do cliente.</p></div>
      ...
      <div class="nr-grid">
        <section class="card nr-card">
          ...
          <div class="nr-tax-config">
            <label>ISSQN %<input type="number" min="0" step="0.01" data-percent="issqnPercent" value="${num(state.issqnPercent)}"></label>
            <label>INSS %<input type="number" min="0" step="0.01" data-percent="inssPercent" value="${num(state.inssPercent)}"></label>
            <label>Simples %<input type="number" min="0" step="0.01" data-percent="simplesPercent" value="${num(state.simplesPercent)}"></label>
          </div>
          ${taxRow('incluirIssqn','ISSQN',state.issqnPercent,r.issqn)}
          ${taxRow('incluirInss','INSS',state.inssPercent,r.inss)}
          ${taxRow('incluirSimples','Simples Nacional',state.simplesPercent,r.simples)}
      ...
      ${r.gross>0?`<section class="card nr-card nr-summary"><div class="card-title"><h3>Resumo do Cálculo</h3></div>
        ...
        ${state.incluirIssqn?`<div class="break-row"><span>(-) ISSQN (${num(state.issqnPercent).toFixed(2)}%)</span><strong class="deduct">- ${money(r.issqn)}</strong></div>`:''}
        ${state.incluirInss?`<div class="break-row"><span>(-) INSS (${num(state.inssPercent).toFixed(2)}%)</span><strong class="deduct">- ${money(r.inss)}</strong></div>`:''}
        ${state.incluirSimples?`<div class="break-row"><span>(-) Simples Nacional (${num(state.simplesPercent).toFixed(2)}%)</span><strong class="deduct">- ${money(r.simples)}</strong></div>`:''}
  ...
  function bind(){
    ...
    section.querySelectorAll('[data-percent]').forEach(el=>el.addEventListener('input',e=>{state[e.target.dataset.percent]=num(e.target.value);save(KEY_STATE,state);render()}));
